...
import { readMywbArchive, writeMywbArchive } from '@mywb/web-adapter/web-archive'
import { WasmSqliteStore } from '@mywb/web-adapter/wasm-sqlite-store'
import type { Editor } from 'tldraw'

// Glue between .mywb bytes and a live tldraw editor. Open extracts the archive,
// reads records out of the WASM sqlite store, rebuilds the {store,schema}
// snapshot the way loadSnapshot expects, and hydrates the editor. Save captures
// the editor snapshot back into a fresh store and re-packs the archive. Assets
// are carried through unchanged so a round-trip is byte-faithful.

export interface LoadedMywb {
	name: string
	assets: Map<string, Uint8Array>
	scriptFiles?: Map<string, string>
	metadata: import('@mywb/core/format').MywbMetadata
}
